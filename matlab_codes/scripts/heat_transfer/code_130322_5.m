...
% Description: Solves a 2D steady state heat transfer problem,
...
title('T (Numerical)')
TA = T(1:Nx,1:Ny);
contourf(X,Y,TA'/1e4,30,'edgecolor','none')
colormap(jet)
colorbar
...
figure(3)
title('T (Exact)')
Texact = 16*X.*Y.*(X-1).*(Y-1);
contourf(X,Y,Texact,30,'edgecolor','none')
colormap(jet)
colorbar
